...
- `workout_sets.target_*` stores the copied plan for one concrete workout session.
...
That gives the frontend editable target values before the user starts logging the actual set results.
...
- target weights are only as reliable as the recent history sent to Gemini
