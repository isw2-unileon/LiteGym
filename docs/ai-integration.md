...
8. The backend parses, normalizes, and returns the routine together with rate-limit metadata.
9. Only a generation log is persisted, so usage can be counted.
...
- if `generated_at` is missing, it uses the current time
- if `generation_source` is missing, it defaults to `"gemini"`

That gives the frontend a more consistent response.

## What Is Stored in the Database

The generated routine is not persisted as a user routine.

What is stored is a generation log in:
...
That log is used only to enforce the usage limit.
...
Authenticated user
  ...
  -> responds with routine + rate limit
```
...
- verify that each `exercise_id` exists in the catalog that was sent
- reject duplicate exercises
...
- it returns a structured routine to the client
